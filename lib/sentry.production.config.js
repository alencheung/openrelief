...
// @sentry/profiling-node is an optional, platform-specific dependency that is
// not currently listed in package.json. Guard the import so the config degrades
// gracefully instead of crashing on load in production.
let ProfilingIntegration = null
try {
  ;({ ProfilingIntegration } = require('@sentry/profiling-node'))
} catch {
  console.warn('@sentry/profiling-node not installed - profiling disabled')
}
...
if (process.env.NODE_ENV === 'production' && SENTRY_DSN) {
  Sentry.init({
    ...
    // 10% of transactions for profiling
    ...
    // Integrations
    integrations: [
      // Only register profiling when @sentry/profiling-node is installed.
      ...
      // Guard the redis import so the config does not crash if the optional
      ...
      ...((() => {
        try {
          ...
          console.warn('redis not installed - Sentry Redis integration disabled')
          return []
        }
      })())
    ],
...
if (typeof window !== 'undefined' && PUBLIC_SENTRY_DSN) {
  Sentry.init({
    ...
    // Sample rates
    // 10% for client-side performance monitoring
    tracesSampleRate: 0.1,
