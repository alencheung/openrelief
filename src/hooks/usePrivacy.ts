...
export const usePrivacy = (options: UsePrivacyOptions = {}) => {
    ...
    (
      ...
      try {
        ...
        if (
          ...
          privacyContext.settings.differentialPrivacy
        ) {
  ...
  const generateTransparencyReport = useCallback(() => {
    ...
    const recentLogs = privacyContext.auditLogs.filter(log => log.timestamp >= thirtyDaysAgo)
    ...
    return {
      ...
      dataRetention: {
        ...
        averageRetentionDays:
          privacyContext.granularPermissions.reduce((sum, p) => sum + p.retentionDays, 0) /
