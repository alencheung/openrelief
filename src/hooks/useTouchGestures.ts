...
export function useTouchGestures(
  ...
  const getMidpoint = useCallback(
    (touch1: Touch, touch2: Touch): TouchPoint => ({
  ...
  const getDirection = useCallback(
    (dx: number, dy: number): 'up' | 'down' | 'left' | 'right' | null => {
    ...
    (event: TouchEvent) => {
      ...
      if (event.touches.length === 2) {
        ...
        initialDistanceRef.current =
          touch1 && touch2 ? calculateDistance(getTouchPoint(touch1), getTouchPoint(touch2)) : 0
        initialAngleRef.current =
          touch1 && touch2 ? calculateAngle(getTouchPoint(touch1), getTouchPoint(touch2)) : 0
      }
  ...
  const handleTouchMove = useCallback(
    (event: TouchEvent) => {
      ...
      if (event.touches.length === 2) {
        ...
        const currentDistance = calculateDistance(getTouchPoint(touch1), getTouchPoint(touch2))
        const currentAngle = calculateAngle(getTouchPoint(touch1), getTouchPoint(touch2))
    ...
    (event: TouchEvent) => {
      ...
      if (distance < options.tapThreshold && event.touches.length === 0) {
        ...
        if (lastTapRef.current) {
          ...
          if (
            timeSinceLastTap < options.doubleTapDelay &&
...
export function useTap(onTap: (point: TouchPoint) => void, options?: Partial<TouchGestureOptions>) {
  return useTouchGestures(
    {
      onTap
    },
