...
export function MotorAccessibility({
  ...
  const updateSetting = useCallback(
    <K extends keyof MotorAccessibilitySettings>(key: K, value: MotorAccessibilitySettings[K]) => {
   ...
   */
  const applyMotorSettingToDocument = useCallback(
    <K extends keyof MotorAccessibilitySettings>(key: K, value: MotorAccessibilitySettings[K]) => {
  ...
  const startScanning = useCallback(() => {
    ...
    const scanElement = (index: number) => {
      ...
      element.focus()
      announcePolite(
        `Scanning: ${element.textContent || element.getAttribute('aria-label') || 'Element ' + (index + 1)}`
      )
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(touchTargetSizes).map(([key, config]) => (
                ...
                >
                  ...
                  <div className="text-xs text-muted-foreground">{config.size}px minimum</div>
                </button>
              ))}
            </div>
          </div>
              ...
              {settings.dwellClicking && (
                <div className="flex items-center justify-between mt-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() =>
                        updateSetting('dwellTime', Math.max(settings.dwellTime - 200, 500))
                      }
                    ...
                    <button
                      onClick={() =>
                        updateSetting('dwellTime', Math.min(settings.dwellTime + 200, 3000))
                      }
                      className="p-1 bg-muted hover:bg-accent rounded"
